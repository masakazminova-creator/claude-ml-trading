...
class TradingBot:
    ...
    async def position(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        try:
            ...
            max_tp_distance = entry_price * (float(os.environ.get("MAX_TAKE_PROFIT_PCT", "1.50")) / 100)
            tp_distance = min(tp_distance, max_tp_distance)
            sl_distance = atr_value * 2.0
            # Floor SL at the same min % the runtime uses
            min_sl_distance = entry_price * (float(os.environ.get("MIN_STOP_LOSS_PCT", "0.80")) / 100)
